...
def compute_orthonormal(args, net, trainset):
    ...
    with torch.no_grad():
        ...
        for m in module_it:    
            ...
            # ============== Nyström替换SVD的核心修改部分 ==============
            # 步骤1: 随机采样列索引 (采样率设为10%)
            n_cols = forward_cov.size(1)
            k = max(64, int(n_cols * 0.1))  # 保证最小采样64列
            
            # 确保在GPU上完成采样
            indices = torch.randperm(n_cols, device=forward_cov.device)[:k]
            C = forward_cov[:, indices]  # 采样列构造子矩阵
            
            # 步骤2: 计算W矩阵并特征分解
            W = C.T @ C
            S_W, U_W = torch.linalg.eigh(W)  # W是Hermitian矩阵
            
            # 步骤3: 构造近似右奇异向量
            mask = S_W > 1e-6  # 过滤数值不稳定的小特征值
            S_W = S_W[mask]
            U_W = U_W[:, mask]
            U_nys = C @ U_W @ torch.diag(1.0 / torch.sqrt(S_W + 1e-8))
            
            # 步骤4: QR分解保证正交性
            Q, R = torch.linalg.qr(U_nys, mode='reduced')
            UT_forward = Q.T  # 近似右奇异向量矩阵的转置
            
            # 保留主成分方向 (可选)
            keep_dim = min(UT_forward.size(0), args.nystrom_keep_dim)  # 可配置参数
            UT_forward = UT_forward[:keep_dim, :]
            # ============== 修改结束 ==============
            ...
            # 保持UT_backward为标准正交基（可选修改）
            UT_backward = torch.eye(weight.shape[0], device=weight.device) 
